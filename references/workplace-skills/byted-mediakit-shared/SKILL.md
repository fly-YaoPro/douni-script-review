...
version: '0.2.0'
license: 'MIT'
description: 'MediaKit 是面向音视频与图像处理的专业工具集，覆盖音视频剪辑与合成、音频媒资探测与人声分离、视频理解与增强、图像增强与内容理解等工作流。用户明确提出叠加、字幕压制、提取字幕、语音转字幕、裁剪、拼接、调速、混音、音视频处理、图片增强或擦除、视频分析或画质增强目标时，先加载本 Skill，再按对象和目标选择 audio、editing、image 或 video；仅说明媒体类型而未说明处理目标时先澄清。不承担具体能力参数说明。'
permissions:
  - shell
metadata:
  requires:
...
- **视频剪辑与合成**：裁剪、拼接与转场、调速、音量调整、画面翻转和滤镜、图片叠加、字幕压制、混音、音视频提取与合流、淡入淡出以及图转视频。
- **音频与音轨处理**：音频或视频音轨的媒资信息探测，以及人声与背景声分离。
- **图像处理与内容理解**：单张图片增强、文字或图标擦除、画质评估、文字识别和背景移除。
- **视频理解与增强**：视频内容理解、剧情或高光分析、从视频提取字幕、语音转字幕、字幕识别与擦除、画质增强、人像或绿幕抠像、媒资探测、场景切分和画面文字识别。
...
| 用户目标 | 优先加载 |
| --- | --- |
| 对现有素材进行裁剪、拼接或转场、调速、叠加、混音、合流或图转视频 | `byted-mediakit-editing` |
| 探测音频媒资信息，或分离音频或视频中的人声与背景声 | `byted-mediakit-audio` |
| 处理单张图片的增强、擦除、画质评估、文字识别或背景移除 | `byted-mediakit-image` |
| 进行视频理解或高光分析、提取字幕、语音转字幕、字幕识别或擦除、画质增强、抠像、媒资探测或场景切分 | `byted-mediakit-video` |
