...
- 命令：`mediakit-cli editing concat-video`
- 生命周期：异步
...
| `transitions` | `--transitions` | array<string> | 否 | - | - | 视频间的转场效果 ID 列表；默认无转场（硬切）。当视频数量超过转场数量 2 个及以上时，系统将自动循环使用转场。 |
...
```bash
mediakit-cli editing concat-video --help
mediakit-cli editing concat-video --schema
```
