...
- 读取可以作为写入前置阶段重复执行。
- 用户附件必须逐一盘点；完整提取结果可以落盘，只把当前任务需要的证据载入上下文。
- 不预先读取无关参考目录，不把参考文件的渐进加载规则用于跳过用户附件。
- 要求的结构读取或全页渲染不可用、失败或结果不完整时，不得声称已经通过；先尝试修复，仍无法完成则报告阻塞。
...
> ⚠️ **禁止**使用外部工具或者 skill 校验本次生成的 Word 文件，如`artifact-preview`，`verifier-hub`，**必须**使用本文提供的「交付验收」流程进行校验。
...
- **确定模板**：若用户明确给定模板，或要求按给定附件格式操作，则用户给定的附件是最高优先级的模板；其次，如果当前任务适用于`writing-taxonomy.md` 中特定体裁推荐的模板，那么应用内置模板执行后续的任务；如果都不适用，那么进行无模板创作。
...
- 以下经验请**严格**遵循
  ...
  - 注意编号的使用，尽量使用自动编号，注意严谨出现自动编号与生成的文本重复
