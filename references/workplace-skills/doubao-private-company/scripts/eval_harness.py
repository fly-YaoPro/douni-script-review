...
FIRST_PARTY_WORDS = re.compile(
    r"官方|公司公告|公司年报|财报|监管|交易所|政府采购|政府|医院|客户|渠道|支付方|招股书|SEC filing|annual report|investor relations",
    re.IGNORECASE,
)
    ...
    if isinstance(sources, list):
        for source in sources:
            if isinstance(source, dict):
                ...
                if re.search(
                    r"first.party|official|regulator|filing|company|exchange|government|procurement|customer|hospital|channel|payer",
                    source_type,
                    re.IGNORECASE,
                ):
                    return True
