...
MEETING_INCREMENTS = {
    "identity_and_technology",
    "revenue_and_cash_collection",
    "clinical_increment",
    "unit_economics",
    "transaction",
}
...
def evaluate_capability_gates(data, config):
    """Evaluate slot-scoped gates without turning local gaps into global errors."""
    capabilities = {}
    for name, spec in config.get("capability_gates", {}).items():
        missing = [
            dotted
            for dotted in spec.get("required", [])
            if not has_value(get_path(data, dotted))
        ]
        blockers = [
            dotted
            for dotted in spec.get("blockers", [])
            if get_path(data, dotted) is True
        ]
        capabilities[name] = {
            "allowed": not missing and not blockers,
            "missing": missing,
            "blockers": blockers,
            "blocked_slots": list(spec.get("blocked_slots", [])),
        }
    return capabilities


def validate_meeting_questions(payload):
    """Require one decision-distinct question for each first-meeting increment."""
    errors = []
    meeting = payload.get("meeting", {})
    questions = meeting.get("kill_questions") if isinstance(meeting, dict) else None
    if questions is None:
        return errors
    if not isinstance(questions, list):
        return ["payload.meeting.kill_questions must be a list"]
    if len(questions) > 5:
        errors.append("payload.meeting.kill_questions exceeds five questions")
    increments = []
    for index, question in enumerate(questions):
        if not isinstance(question, dict):
            errors.append(f"payload.meeting.kill_questions[{index}] invalid")
            continue
        for field in ("question", "information_increment", "why_it_matters", "upgrade_if", "downgrade_if"):
            if not has_value(question.get(field)):
                errors.append(
                    f"payload.meeting.kill_questions[{index}].{field} missing"
                )
        increment = question.get("information_increment")
        if increment and increment not in MEETING_INCREMENTS:
            errors.append(
                f"payload.meeting.kill_questions[{index}].information_increment invalid"
            )
        increments.append(increment)
    present = {item for item in increments if item in MEETING_INCREMENTS}
    if len(present) != len(increments):
        errors.append("payload.meeting.kill_questions must use distinct information increments")
    missing = sorted(MEETING_INCREMENTS - present)
    if missing:
        errors.append(
            "payload.meeting.kill_questions missing increments: " + ",".join(missing)
        )
    coverage_complete = not missing and len(questions) == len(MEETING_INCREMENTS)
    if meeting.get("coverage_complete") is not coverage_complete:
        errors.append(
            "payload.meeting.coverage_complete must reflect complete five-increment coverage"
        )
    return errors
...
def main():
    ...
    errors.extend(validate_meeting_questions(payload))
    ...
    capabilities = evaluate_capability_gates(data, config)
    for name, gate in capabilities.items():
        if not gate["allowed"]:
            details = []
            if gate["missing"]:
                details.append("missing=" + ",".join(gate["missing"]))
            if gate["blockers"]:
                details.append("blockers=" + ",".join(gate["blockers"]))
            warnings.append(f"{name}=false {' '.join(details)}")
