...
def main():
    ...
    assert template["precedent_parameter_inheritance_allowed"] is False
    ...
    assert template["tier_table_may_contain_absolute_amounts"] is False
    ...
    assert listing["must_search_tender_offer_privatization_delisting_per_entity"] is True
    assert listing["industry_common_knowledge_is_sufficient"] is False
    assert template["absolute_amount_ban_applies_to_whole_response"] is True
    assert template["tier_table_replaces_stress_scenario"] is False
