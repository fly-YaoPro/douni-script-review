...
def main():
    ...
    assert "name: doubao-wealth-planning" in skill
    seed=config["search_runtime"]["provider_routing"]["seed_finance_search"]
    assert seed["priority"]=="low" and seed["unavailable_fallback"]=="general_search"
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        manifest=json.loads(report.with_name("report-manifest.json").read_text(encoding="utf-8"))
        assert manifest["source_locked"] is True
        assert manifest["finalizer_run_count"]==1
        assert manifest["facts_sha256_before"]==manifest["facts_sha256_after"]
        assert not report.with_suffix(".docx").exists()
    print("PASS wealth-planning-seed-finance V3");return 0
