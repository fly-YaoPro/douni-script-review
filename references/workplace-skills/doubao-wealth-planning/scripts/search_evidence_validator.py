...
def validate(payload):
    ...
    for index, claim in enumerate(claims):
        ...
        if claim.get("source_role") not in SOURCE_ROLES:
            errors.append(f"{claim_id}: invalid source_role")
        if database_evidence:
            if claim.get("source_role") != "database_aggregator":
                errors.append(
                    f"{claim_id}: database evidence must use "
        ...
        if claim.get("claim_kind") == "rule" and claim.get("source_role") not in {
            "government",
            "regulator",
            "official_program",
        }:
            errors.append(
                f"{claim_id}: rule claim requires government/regulator/"
                "official_program source role"
            )
